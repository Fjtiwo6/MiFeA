...
mifea = Token:match("(%d+)")
redis:set(mifea..":VERSION",2.9)
redis:set(mifea..":SUDO_ID:",SUDO_USER)
redis:set(mifea..":DataCenter:",'German')
redis:set(mifea..":UserNameBot:",BOT_User)
redis:set(mifea..":NameBot:",BOT_NAME)
redis:hset(mifea..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3mifea_INSTALL","Yes")
info = {}
...
mifea = Token:match("(%d+)")
our_id = tonumber(mifea)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(mifea..":UserNameBot:")
SUDO_ID = tonumber(redis:get(mifea..":SUDO_ID:"))
SUDO_USER = redis:hgetall(mifea..'username:'..SUDO_ID).username
version = redis:get(mifea..":VERSION")
DataCenter = redis:get(mifea..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..mifea..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(mifea..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(mifea..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(mifea..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(mifea..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(mifea..':MALK_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المـــــــــالك💥'
msg.TheRank = 'المـــــــــالك💥 '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(mifea..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي👷🏽'
msg.TheRank = 'المنشىء الاساسي👷🏽 '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(mifea..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(mifea..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(mifea..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 7
elseif msg.GroupActive and redis:sismember(mifea..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
if msg.content_.ID == "MessageChatAddMembers" then
	local lock_bots = redis:get(mifea..'lock_bots'..msg.chat_id_)
	ZISBOT = false
	...
	if msg.GroupActive and ZISBOT and redis:get(mifea..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
	if msg.content_.members_[0].id_ == our_id and redis:get(mifea..':WELCOME_BOT') then
	SUDO_USER = redis:hgetall(mifea..'username:'..SUDO_ID).username
	sendPhoto(msg.chat_id_,msg.id_,redis:get(mifea..':WELCOME_BOT'),[[🙋🏽‍♂╿ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(mifea..':NameBot:')..[[ ⚜
	...
	if msg.GroupActive and redis:get(mifea..'mute_tgservice'..msg.chat_id_) then
	Del_msg(msg.chat_id_,msg.id_)
	return false 
	end
	...
	if msg.Admin then redis:srem(mifea..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
	...
	if msg.Admin then redis:srem(mifea..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
	...
	if redis:get(mifea..'mute_text'..msg.chat_id_) then --قفل الدردشه
	...
	if ScriptFile and ScriptFile.mifea then 
	if msg.text and ScriptFile.imifea then
	for k, mifea in pairs(ScriptFile.mifea) do
	local SearchText = msg.text:match(mifea)
	if SearchText then
	if not CheckFlood(msg) then
	return false 
	end
	
	local GetMsg = ScriptFile.imifea(msg,{msg.text:match(mifea)})
	if GetMsg then
	print("\27[1;35m¦This_Msg : ",mifea.." | Plugin is: \27[1;32mScript.lua\27[0m")
	...
	if ScriptFile.dmifea then
	if ScriptFile.dmifea(msg) == false then
	return false
	end
	...
	if Plug.mifea then 
	if msg.text and Plug.imifea then
	for k, mifea in pairs(Plug.mifea) do
	local SearchText = msg.text:match(mifea)
	if SearchText then
	if not CheckFlood(msg) then
	return false
	...
	Mohammad = msg.text
	if Mohammad then
	if redis:sismember(mifea..'CmDlist:'..msg.chat_id_,Mohammad) then
	mmdi = redis:hget(mifea..'CmD:'..msg.chat_id_,Mohammad)
	msg.text = Mohammad:gsub(Mohammad,mmdi)
	end
	end
	local GetMsg = Plug.imifea(msg,{msg.text:match(mifea)})
	if GetMsg then
	print("\27[1;35m¦This_Msg : ",mifea.." | Plugin is: \27[1;32m"..name.."\27[0m")
	sendMsg(msg.chat_id_,msg.id_,GetMsg)
	end 
	return false
	end
	...
	print("The File "..name.." Not Runing in The Source mifea")
	end 
	
	end
	else
	print("The File Script.lua Not Runing in The Source mifea")
...
{text = '⓺', callback_data="/help6@"..data.sender_user_id_},{text = '⓻', callback_data="/help7@"..data.sender_user_id_},
	  ...
	  answerCallbackQuery(data.id_,'قائمة الاوامر ليست لك', true)
		...
		redis:srem(mifea..'users',msg.sender_user_id_)
		end
		...
		download_file('https://raw.githubusercontent.com/Fjtiwo6/MiFeA/master/inc/Run.lua','./inc/Run.lua')
		EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
		download_file('https://raw.githubusercontent.com/Fjtiwo6/MiFeA/master/inc/locks.lua','./inc/locks.lua')
		EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
		download_file('https://raw.githubusercontent.com/Fjtiwo6/MiFeA/master/inc/Script.lua','./inc/Script.lua')
		EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
		download_file('https://raw.githubusercontent.com/Fjtiwo6/MiFeA/master/inc/functions.lua','./inc/functions.lua')
		download_file('https://raw.githubusercontent.com/Fjtiwo6/MiFeA/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
		download_file('https://raw.githubusercontent.com/Fjtiwo6/MiFeA/master/plugins/games.lua','./plugins/games.lua')
	...
	msg.text = Mohammad:gsub(Mohammad,mmdi)
		...
		mmd = redis:get(mifea..":SUDO_ID:")
	redis:hset(mifea..'username:'..tonumber(mmd),'username',msg.text)
		...
		redis:set(mifea..":SUDO_ID:",msg.text)
	...
	download(stk,32)
		...
		if redis:get(mifea..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(mifea..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(mifea..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(mifea..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(mifea..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
		msg.Admin = true
		end
		local lock_bots = redis:get(mifea..'lock_bots'..msg.chat_id_)
		ISBOT = false
		...
		if ZISBOT and redis:get(mifea..'lock_bots_by_kick'..msg.chat_id_) then
		kick_user(msg.sender_user_id_, msg.chat_id_)
		end
	...
	if redis:get(mifea..'lock:join:'..msg.chat_id_) then
	kick_user(msg.sender_user_id_,msg.chat_id_)
		...
		if redis:get(mifea..'group:add'..data.chat_.id_) then
		redis:set(mifea..'group:name'..data.chat_.id_,data.chat_.title_)
		end
		...
		if redis:get(mifea..'group:add-100'..data.channel_.id_) then
		local linkGroup = (redis:get(mifea..'linkGroup-100'..data.channel_.id_) or "")
		local NameGroup = (redis:get(mifea..'group:name-100'..data.channel_.id_) or "")
		...
		redis:mset(
		mifea..'group:name'..k,IDS.Title,
		mifea..'num_msg_mifea'..k,5,
		mifea..'group:add'..k,true,
		mifea..'lock_link'..k,true,
		mifea..'lock_spam'..k,true,
		mifea..'lock_webpage'..k,true,
		mifea..'lock_markdown'..k,true,
		mifea..'lock_flood'..k,true,
		mifea..'lock_bots'..k,true,
		mifea..'mute_forward'..k,true,
		mifea..'mute_contact'..k,true,
		mifea..'mute_document'..k,true,
		mifea..'mute_inline'..k,true,
		mifea..'lock_username'..k,true,
		mifea..'replay'..k,true
		)
		redis:sadd(mifea..'group:ids',k) 
	
		if IDS.Admins then
		for user,ID in pairs(IDS.Admins) do
		redis:hset(mifea..'username:'..ID,'username',user)
		redis:sadd(mifea..'admins:'..k,ID)
		end
		end
		if IDS.Creator then
		for user,ID in pairs(IDS.Creator) do
		redis:hset(mifea..'username:'..ID,'username',user)
		redis:sadd(mifea..':MONSHA_BOT:'..k,ID)
		end
		end
		if IDS.Owner then
		for user,ID in pairs(IDS.Owner) do
		redis:hset(mifea..'username:'..ID,'username',user)
		redis:sadd(mifea..'owners:'..k,ID)
		end
		end
		end
		io.popen("rm -fr ../.telegram-cli/data/document/*")
		sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(mifea..'group:ids')..'* 🌿\n✓')
		end
		elseif data.ID == "UpdateUser" then  
		if data.user_.type_.ID == "UserTypeDeleted" then
		print("¦ userTypeDeleted")
		redis:srem(mifea..'users',data.user_.id_)
		elseif data.user_.type_.ID == "UserTypeGeneral" then
		local CheckUser = redis:hgetall(mifea..'username:'..data.user_.id_).username
		...
		redis:hset(mifea..'username:'..data.user_.id_,'username',USERNAME)
		end 
		end
